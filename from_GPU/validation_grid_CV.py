...
if __name__ == "__main__":
    ...
    if tpe is 2:
        loss_all_m = lasagne.objectives.squared_error(output_m * x_sym, m_sym) \
                     - gamma * lasagne.objectives.squared_error(output_m * x_sym, f_sym)
        loss_all_f = lasagne.objectives.squared_error(output_f * x_sym, f_sym) \
            - gamma * lasagne.objectives.squared_error(output_f * x_sym, m_sym)
        loss_mean_m = T.mean(loss_all_m)
        loss_mean_f = T.mean(loss_all_f)
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/mse_GRID_{}_N_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/mse_GRID_{}_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/mse_GRID_m_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/mse_GRID_f_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/one_minus_GRID_{}_N_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/one_minus_GRID_{}_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/one_minus_GRID_m_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/one_minus_GRID_f_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/force_SIR_GRID_{}_N_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/force_SIR_GRID_{}_gamma_{}.mat'.format(index_timit, gamma), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/force_SIR_GRID_m_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/force_SIR_GRID_f_{}_{}_{}_gamma_{}'.format(date, NUM_UNITS_ENC, index_timit, gamma), 'wb'))
