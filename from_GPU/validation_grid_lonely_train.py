...
if __name__ == "__main__":
    ...
    for i in range(epochs):
        ...
        for j in range(10):
            ...
            loss_m, _ = train_model_m(batch_m, batch_m, batch_f)
            loss_train_m += loss_m

            if tpe is not 1:
                loss_f, _ = train_model_f(batch_f, batch_f, batch_m)
                loss_train_f += loss_f
        ...
        loss_test_m, out_m = test_model_m(batch_test_m, batch_test_m, batch_test_f)
        if tpe is not 1:
            loss_test_f, out_f = test_model_f(batch_test_f, batch_test_f, batch_test_m)
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/lonely_mse_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/lonely_mse_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/lonely_mse_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/lonely_mse_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/lonely_one_minus_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/lonely_one_minus_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/lonely_one_minus_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/lonely_one_minus_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/lonely_force_SIR_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/lonely_force_SIR_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/lonely_force_SIR_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/lonely_force_SIR_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
