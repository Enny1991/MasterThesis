...
if __name__ == "__main__":
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/mse_grid_{}_N_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/mse_grid_{}_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_m]),
                 open('models/mse_grid_m_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_f]),
                 open('models/mse_grid_f_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/one_minus_grid_{}_N_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/one_minus_grid_{}_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_m]),
                 open('models/one_minus_grid_m_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_f]),
                 open('models/one_minus_grid_f_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/force_SIR_grid_{}_N_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/force_SIR_grid_{}_pca_direct.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_m]),
                 open('models/force_SIR_grid_m_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_f]),
                 open('models/force_SIR_grid_f_{}_{}_{}_pca_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))

    if tpe is 3:
        if normalize is 1:
            savemat('results/real_mse_grid_{}_N_direct.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/real_mse_grid_{}_direct.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_m]),
                 open('models/real_mse_grid_m_{}_{}_{}_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_decoder_f]),
                 open('models/real_mse_grid_f_{}_{}_{}_direct'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
