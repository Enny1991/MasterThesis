...
if __name__ == "__main__":
    ...
    # slices are
    sl_b = [0, 42, 84, 126, 168, 208]
    sl_e = [60, 102, 144, 186, 228, 257]
    num_min_batches = 100
    n_batch = 100
    epochs = 75
    for e in range(epochs):
        ...
        for j in range(10):
            ...
            for i in range(6):
                ...
                slice_m = batch_m[:, :, sl_b[i]:sl_e[i]]
                slice_f = batch_f[:, :, sl_b[i]:sl_e[i]]
                slice_x = batch_x[:, :, sl_b[i]:sl_e[i]]
        ...
        for i in range(6):
            slice_m = batch_test_m[:, :, sl_b[i]:sl_e[i]]
            slice_f = batch_test_f[:, :, sl_b[i]:sl_e[i]]
            slice_x = batch_test_x[:, :, sl_b[i]:sl_e[i]]
    ...
    out_out_m = np.zeros_like(test_x)
    out_out_f = np.zeros_like(test_x)
    for i in range(6):
        slice_m = test_m[:, :, sl_b[i]:sl_e[i]]
        slice_f = test_f[:, :, sl_b[i]:sl_e[i]]
        slice_x = test_x[:, :, sl_b[i]:sl_e[i]]
        test_model_m = TEST_M[i]
        l_m, out_m = test_model_m(slice_x, slice_m, slice_f)
        out_out_m[:, :, sl_b[i]:sl_e[i]] += out_m
        print 'M TEST = %.10f' % l_m

        if tpe is not 1:
            test_model_f = TEST_F[i]
            l_f, out_f = test_model_f(slice_x, slice_f, slice_m)
            out_out_f[:, :, sl_b[i]:sl_e[i]] += out_f
            print 'F TEST = %.10f' % l_f
    for i in range(5):
	out_out_m[:, :, sl_b[i+1]:sl_e[i]] /= 2
	out_out_f[:, :, sl_b[i+1]:sl_e[i]] /= 2
