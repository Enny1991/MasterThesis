...
if __name__ == "__main__":
    ...
    # I load all 6 GRID Databases
    fs = 12 #kHz
    train_m = np.zeros((0, 257))
    train_f = np.zeros((0, 257))
    train_x = np.zeros((0, 257))
    test_m = np.zeros((0, 257))
    test_f = np.zeros((0, 257))
    test_x = np.zeros((0, 257))
    list_t = np.concatenate((np.arange(1,index_timit),np.arange(index_timit+1,7)))
    for i in range(5):
        train_m = np.concatenate((train_m, loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, list_t[i]))['PWRa']))
        train_f = np.concatenate((train_f, loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, list_t[i]))['PWRb']))
        train_x = np.concatenate((train_x, loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, list_t[i]))['PWR']))

    # test
    fine_train_m = loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, index_timit))['PWRa']
    fine_train_f = loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, index_timit))['PWRb']
    fine_train_x = loadmat('data/{}kHz_GRID/GRID_{}.mat'.format(fs, index_timit))['PWR']
    l_t = fine_train_m.shape[0]
    l_t_t = int(l_t / 100)
    fine_train_m = fine_train_m[:l_t_t]
    fine_train_f = fine_train_f[:l_t_t]
    fine_train_x = fine_train_x[:l_t_t]
    print fine_train_x.shape
    ...
    n_batch = 100
    epochs = 30
    train_m = fine_train_m
    train_f = fine_train_f
    train_x = fine_train_x
    for i in range(epochs):
        start_time = time.time()
        loss_train_m = 0
        loss_train_f = 0
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/fine_tune_mse_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/fine_tune_mse_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/fine_tune_mse_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/fine_tune_mse_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/fine_tune_one_minus_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/fine_tune_one_minus_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/fine_tune_one_minus_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/fine_tune_one_minus_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/fine_tune_force_SIR_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/fine_tune_force_SIR_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/fine_tune_force_SIR_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/fine_tune_force_SIR_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
