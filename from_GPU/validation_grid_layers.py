...
if __name__ == "__main__":
    ...
    train_m = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWRa']
    train_f = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWRb']
    train_x = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWR']
    test_m = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWRatest']
    test_f = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWRbtest']
    test_x = loadmat('data/12kHz_GRID/GRID_{}.mat'.format(index_timit))['PWRtest']
    ...
    # Adding craziness
    l_more_1 = lasagne.layers.GRULayer(l_concat, num_units=NUM_UNITS_DEC)
    l_more_2 = lasagne.layers.GRULayer(l_more_1, num_units=NUM_UNITS_DEC)
    #
    ...
    # Adding craziness part 2
    l_more_1_2_m = lasagne.layers.GRULayer(l_decoder_m, num_units=NUM_UNITS_DEC)
    l_more_2_2_m = lasagne.layers.GRULayer(l_more_1_2_m, num_units=NUM_UNITS_DEC)
    l_more_1_2_f = lasagne.layers.GRULayer(l_decoder_f, num_units=NUM_UNITS_DEC)
    l_more_2_2_f = lasagne.layers.GRULayer(l_more_1_2_f, num_units=NUM_UNITS_DEC)
    #

    l_reshape_m = lasagne.layers.ReshapeLayer(l_more_2_2_m, (-1, NUM_UNITS_DEC))
    ...
    l_reshape_f = lasagne.layers.ReshapeLayer(l_more_2_2_f, (-1, NUM_UNITS_DEC))
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/more_mse_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/more_mse_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/more_mse_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/more_mse_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/more_one_minus_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/more_one_minus_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/more_one_minus_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/more_one_minus_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/more_force_SIR_GRID_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/more_force_SIR_GRID_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/more_force_SIR_GRID_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/more_force_SIR_GRID_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
