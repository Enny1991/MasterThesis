...
l_plus_m = lasagne.layers.ConcatLayer([l_decoder_m, l_encoder_2_m], axis=2)
l_plus_f = lasagne.layers.ConcatLayer([l_decoder_f, l_encoder_2_f], axis=2)

l_reshape_m = lasagne.layers.ReshapeLayer(l_plus_m, (-1, NUM_UNITS_DEC*2))
l_dense_m = lasagne.layers.DenseLayer(l_reshape_m, num_units=n_features, nonlinearity=identity)
l_out_m = lasagne.layers.ReshapeLayer(l_dense_m, (-1, max_len, n_features))

l_reshape_f = lasagne.layers.ReshapeLayer(l_plus_f, (-1, NUM_UNITS_DEC*2))
l_dense_f = lasagne.layers.DenseLayer(l_reshape_f, num_units=n_features, nonlinearity=identity)
...
loss_all_target_m = lasagne.objectives.squared_error(output_m * x_sym, t_sym)

loss_mean_target_m = T.mean(loss_all_target_m)

loss_all_target_f = lasagne.objectives.squared_error(output_f * x_sym, t_sym)
...
savemat('RR_2models_PCA24_dense_mask.mat', {'out_out_m': out_out_m,
...
pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
         open('RR_2models_PCA24_dense_m_mask'.format(date, NUM_UNITS_ENC), 'wb'))
pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
         open('RR_2models_PCA24_dense_f_mask'.format(date, NUM_UNITS_ENC), 'wb'))
