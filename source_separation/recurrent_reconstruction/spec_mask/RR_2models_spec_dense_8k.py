...
def create_batches(n_samples, train=True):
    ...
    if train:
        ...
        sel_f = test_f
        show = True
        batch_x = np.zeros((100, max_len, n_features)).astype(theano.config.floatX)
        batch_m = np.zeros((100, max_len, n_features)).astype(theano.config.floatX)
        batch_f = np.zeros((100, max_len, n_features)).astype(theano.config.floatX)
        for i in range(100):
...
data_m = loadmat('../../../data/m_f_book_SPEC8.mat')['hunchback']
data_f = loadmat('../../../data/m_f_book_SPEC8.mat')['screw']
data_mix = loadmat('../../../data/m_f_book_SPEC8.mat')['hunchback_screw']
