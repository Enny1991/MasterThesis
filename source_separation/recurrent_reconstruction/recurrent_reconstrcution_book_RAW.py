...
def create_batches(n_samples, train=True):
    ...
    if train:
        ...
        sel_f = test_f
        show = True
        batch_x = np.zeros((n_samples_test, max_len, n_features))
        batch_t = np.zeros((n_samples_test, max_len, n_features * 2))
...
data_m = loadmat('../../data/m_f_book_raw.mat')['hunchback']
data_f = loadmat('../../data/m_f_book_raw.mat')['screw']
data_mix = loadmat('../../data/m_f_book_raw.mat')['hunchback_screw']
...
for i in range(epochs):
    ...
    for j in range(10):
        ...
        print 'loss batch %.4f' % loss_train_target

    batch_test_x, batch_test_t = create_batches(100, False)
    loss_test, out = test_model(batch_test_x[:100], batch_test_t[:100])
...
out = np.zeros_like(test_t)
for k in range(446):
    l, out2 = test_model(test_x[k*max_len:(k+1)*max_len], test_t[k*max_len:(k+1)*max_len])
    out[k*max_len:(k+1)*max_len] = out2

out_out = out

print 'TEST = %.6f' % l

savemat('recurrent_reconstruction_PCA_no_dense_24.mat', {'out_out': out_out})
