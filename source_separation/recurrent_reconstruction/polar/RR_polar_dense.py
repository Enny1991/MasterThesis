...
l_reshape = lasagne.layers.ReshapeLayer(l_decoder, (-1, n_features*2))
l_dense = lasagne.layers.DenseLayer(l_reshape, num_units=n_features * 2, nonlinearity=identity)
l_out = lasagne.layers.ReshapeLayer(l_dense, (-1, max_len, n_features * 2))


output = lasagne.layers.get_output(l_out, inputs={l_in: x_sym})
...
savemat('RR_polar_dense.mat', {'out_out': out_out,
...
pkl.dump(lasagne.layers.get_all_param_values([l_out]),
         open('RR_polar_dense'.format(date, NUM_UNITS_ENC), 'wb'))
