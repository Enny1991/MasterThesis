...
if __name__ == "__main__":
    ...
    def create_batches(n_samples, train=True):
        ...
        if train:
            ...
            sel_f = train_f
	    sel_as = train_as
	    batch_as = np.zeros((n_samples, max_len, n_features+256)).astype(theano.config.floatX)
            ...
            idx = np.random.permutation(sel_as.shape[0] - max_len)
            beg = idx[:n_samples]
            for i, b in enumerate(beg):
                batch_as[i,:,:256] = sel_as[b:b + max_len]
		batch_as[i,:,256:] = sel_mix[b:b + max_len]
            ...
            sel_f = test_f
 	    sel_as = test_as
            Q = int(sel_as.shape[0] / max_len)
	    batch_as = np.zeros((Q, max_len, n_features+256)).astype(theano.config.floatX)
            ...
            for i in range(Q):
		batch_as[i,:,:256] = sel_as[i*max_len:((i+1)*max_len)]
		batch_as[i,:,256:] = sel_mix[i*max_len:((i+1)*max_len)]
                batch_x[i] = sel_mix[i*max_len:((i+1)*max_len)]
                batch_m[i, :, :] = sel_m[i*max_len:((i+1)*max_len)]
                batch_f[i, :, :] = sel_f[i*max_len:((i+1)*max_len)]
        return batch_x, batch_m, batch_f, batch_as

    fs = 16

    train_as = loadmat('data/TSP_{}_AS.mat'.format(index_timit))['ASP']
    print train_as.shape 
    ...
    test_as = loadmat('data/TSP_{}_AS.mat'.format(index_timit))['ASPtest']
    print test_as.shape 
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, max_len, n_features+256))
    ...
    output_m = lasagne.layers.get_output(l_out_m, inputs={l_in: x_as_sym})
    output_f = lasagne.layers.get_output(l_out_f, inputs={l_in: x_as_sym})
    output_m_test = lasagne.layers.get_output(l_out_m, inputs={l_in: x_as_sym}, deterministic=True)
    output_f_test = lasagne.layers.get_output(l_out_f, inputs={l_in: x_as_sym}, deterministic=True)
    ...
    train_model_m = theano.function([x_sym, m_sym, f_sym, x_as_sym],
                                    [loss_mean_m, output_m],
                                    updates=updates_target_m,
                                    on_unused_input='ignore')

    test_model_m = theano.function([x_sym, m_sym, f_sym, x_as_sym],
    ...
    for i in range(epochs):
        ...
        loss_train_f = 0
        for j in range(10):
            batch_x, batch_m, batch_f, batch_as = create_batches(n_batch)
            loss_m, _ = train_model_m(batch_x, batch_m, batch_f, batch_as)
            loss_train_m += loss_m

            if tpe is not 1:
                loss_f, _ = train_model_f(batch_x, batch_f, batch_m, batch_as)
                loss_train_f += loss_f
        print 'M loss %.10f' % (loss_train_m / 10)
        if tpe is not 1:
            print 'F loss %.10f' % (loss_train_f / 10)

        batch_test_x, batch_test_m, batch_test_f, batch_test_as = create_batches(100, False)
        loss_test_m, out_m = test_model_m(batch_test_x, batch_test_m, batch_test_f, batch_test_as)
        if tpe is not 1:
            loss_test_f, out_f = test_model_f(batch_test_x, batch_test_f, batch_test_m, batch_test_as)
    ...
    # final test
    test_x, test_m, test_f, test_as = create_batches(100, False)
    l_m, out_m = test_model_m(test_x, test_m, test_f, test_as)
    out_out_m = out_m
    print 'TRAIN M = %.10f' % (loss_train_m / 10)
    print 'TEST M = %.10f' % l_m

    if tpe is not 1:
        l_f, out_f = test_model_f(test_x, test_f, test_m, test_as)
        out_out_f = out_f
    ...
    type_tr = ''
    pref = 'AS_{}_{}_{}_{}_'.format(p,NUM_UNITS_ENC,NUM_UNITS_DEC,third_layer)
    postf = ''
    if tpe is 0:
        savemat('results/tsp/AS_{}_{}_{}_{}_special_TSP_{}.mat'.format(p,NUM_UNITS_ENC,NUM_UNITS_DEC,third_layer,index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        type_tr = 'special'
    if tpe is 1:
        savemat('results/tsp/AS_{}_{}_{}_{}_one_minus_TSP_{}.mat'.format(p,NUM_UNITS_ENC,NUM_UNITS_DEC,third_layer,index_timit), {'out_out_m': out_out_m})
        type_tr = 'one_minus'
    if tpe is 2:
        savemat('results/tsp/AS_{}_{}_{}_{}_force_SIR_TSP_{}.mat'.format(p,NUM_UNITS_ENC,NUM_UNITS_DEC,third_layer,index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        type_tr = 'force'
    if tpe is 3:
        savemat('results/tsp/AS_{}_{}_{}_{}_mse_TSP_{}.mat'.format(p,NUM_UNITS_ENC,NUM_UNITS_DEC,third_layer,index_timit),{'out_out_m':out_out_m,'out_out_f':out_out_f})
        type_tr = 'mse'
    ...
    table = [["SNR", SNR[0][0], SNR[1][0], (SNR[0][0] + SNR[1][0])/2],
             ["SIR", SIR[0][0], SIR[1][0], (SIR[0][0] + SIR[1][0])/2],
             ["SDR", SDR[0][0], SDR[1][0], (SDR[0][0] + SDR[1][0])/2],
             ["SAR", SAR[0][0], SAR[1][0], (SAR[0][0] + SAR[1][0])/2],
             ["PESQ", PESQ[0][0], PESQ[1][0], (PESQ[0][0] + PESQ[1][0])/2],
             ["STOI", STOI[0][0], STOI[1][0], (STOI[0][0] + STOI[1][0])/2]]
    headers = ["Measurements", "Male", "Female","Average"]
