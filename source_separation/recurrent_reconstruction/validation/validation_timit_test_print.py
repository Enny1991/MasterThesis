...
if __name__ == "__main__":
    ...
    train_m = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWRa']
    train_f = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWRb']
    train_x = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWR']
    test_m = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWRatest']
    test_f = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWRbtest']
    test_x = loadmat('data/classic_timit_{}.mat'.format(index_timit))['PWRtest']
    ...
    for i in range(epochs):
        ...
        print ('-'*5 + ' epoch = %2i ' + '-'*5 + ' time = %.4f ' + '-'*5) % (i, stop_time)
        print ('|----------------| Male Loss   | Female Loss |')
        print ('|  Previous Train|    %.4f    %.4f    |') % (last_m, last_f)
        print ('|--------------------------------------------|')
        if tpe is not 1:
            print ('|  Current  Train|    %.4f    %s    |') % (loss_train_m / 10, '-----')
        print ('|  Previous Test|    %.4f    %.4f    |') % (last_m, last_f)
        print ('|--------------------------------------------|')
        if tpe is not 1:
            print ('|  Current  Test|    %.4f    %s    |') % (loss_train_m / 10, '-----')
    ...
    if tpe is 0:
        if normalize is 1:
            savemat('results/mse_timit_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/mse_timit_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/mse_timit_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/mse_timit_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 1:
        if normalize is 1:
            savemat('results/one_minus_timit_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                                     'mmm': mmm})
        else:
            savemat('results/one_minus_timit_{}.mat'.format(index_timit), {'out_out_m': out_out_m})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/one_minus_timit_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/one_minus_timit_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
    if tpe is 2:
        if normalize is 1:
            savemat('results/force_SIR_timit_{}_N.mat'.format(index_timit), {'out_out_m': out_out_m,
                                                               'out_out_f': out_out_f,
                                                               'mmm': mmm})
        else:
            savemat('results/force_SIR_timit_{}.mat'.format(index_timit), {'out_out_m': out_out_m, 'out_out_f': out_out_f})
        pkl.dump(lasagne.layers.get_all_param_values([l_out_m]),
                 open('models/force_SIR_timit_m_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
        pkl.dump(lasagne.layers.get_all_param_values([l_out_f]),
                 open('models/force_SIR_timit_f_{}_{}_{}'.format(date, NUM_UNITS_ENC, index_timit), 'wb'))
