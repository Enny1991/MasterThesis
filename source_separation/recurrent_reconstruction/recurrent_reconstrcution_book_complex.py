...
data_m = loadmat('../../data/m_f_book_comp.mat')['hunchback']
data_f = loadmat('../../data/m_f_book_comp.mat')['screw']
data_mix = loadmat('../../data/m_f_book_comp.mat')['hunchback_screw']

data_mix /= np.max(np.abs(data_mix))
data_f /= np.max(np.abs(data_f))
data_m /= np.max(np.abs(data_m))
print np.max(np.abs(data_m))
print np.max(np.abs(data_f))
# find max
...
print lasagne.layers.get_output(l_decoder, inputs={l_in: x_sym}).eval({x_sym: ftest_x}).shape
...
savemat('recurrent_reconstruction_PCA_comp.mat', {'out_out': out_out})
date = time.strftime("%H:%M_%d:%m:%Y")
pkl.dump(lasagne.layers.get_all_param_values([l_decoder]),
          open('recurrent_reconstruction_PCA_{}_{}_comp'.format(date, NUM_UNITS_ENC), 'wb'))
